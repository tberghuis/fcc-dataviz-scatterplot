...
var _cyclistData = __webpack_require__(1);

var _cyclistData2 = _interopRequireDefault(_cyclistData);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function _interopRequireWildcard(obj) { if (obj && obj.__esModule) { return obj; } else { var newObj = {}; if (obj != null) { for (var key in obj) { if (Object.prototype.hasOwnProperty.call(obj, key)) newObj[key] = obj[key]; } } newObj.default = obj; return newObj; } }

d3.tip = __webpack_require__(2);
...
var xScale = d3.scaleLinear().domain([d3.max(_cyclistData2.default, xValue) + 5, 0]) // +5 add some space
.range([0, width]);

var yScale = d3.scaleLinear().domain([1, d3.max(_cyclistData2.default, yValue) + 1]).range([0, height]);

var svg = d3.select('.chart').append("svg").attr('width', WIDTH).attr('height', HEIGHT);

var rootGroup = svg.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

rootGroup.selectAll("circle").data(_cyclistData2.default).enter().append("circle").attr("cx", function (d) {
...
rootGroup.selectAll("text").data(_cyclistData2.default).enter().append("text").text(function (d) {
    return d.Name;
}).attr("x", function (d) {
    return xScale(xValue(d));
}).attr("y", function (d) {
    return yScale(yValue(d));
}).attr("transform", "translate(10,+4)").attr("data-place", function (d) {
    return yValue(d);
});
...
legend.append("circle").attr("cx", function (d) {
    return 10;
}).attr("cy", function (d) {
    return 10;
...
rootGroup.append("g").attr("transform", "translate(0," + height + ")").call(xAxis);
rootGroup.append("text").attr("transform", "translate(" + width / 2 + " ," + (height + margin.top + 40) + ")").style("text-anchor", "middle").text("Seconds behind fastest time");

//left
rootGroup.append("g").call(d3.axisLeft(yScale));

rootGroup.append("text").attr("transform", "rotate(-90)").attr("y", -60).attr("x", 0 - height / 2).attr("dy", "1em").style("text-anchor", "middle").text("Ranking");
...
rootGroup.append("g").call(tip);

// mouseover
rootGroup.selectAll("[data-place]").on("mouseover", function (d) {
    //text black
